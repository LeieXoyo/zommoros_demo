...
class CCXTFeed(with_metaclass(MetaCCXTFeed, DataBase)):
    ...
    def _load_ticks(self):
        ...
        for trade in trades:
            ...
            price = float(trade['info']['p'])
            high = price if high == 0 or price > high else high
            low = price if low == 0 or price < low else low
            volume += float(trade['info']['q'])
            self._last_id = trade_id

        self._data.append((trade_time, price, high, low, volume))
        ...
        trade_time, price, high, low, volume = trade
        ...
        self.lines.volume[0] = round(volume, 3)
