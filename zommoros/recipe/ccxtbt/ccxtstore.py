...
class CCXTStore(with_metaclass(MetaSingleton, object)):
    ...
    def getposition(self, symbol):
        pos_list = list(filter(lambda i: i['symbol'] == symbol, self.exchange.fetch_positions()))
        if pos_list:
            return pos_list[0]
        else:
            raise Exception(f'无法获取{symbol}的仓位信息.')
    
    @retry
    def getinfo(self, symbol):
        info_list = list(filter(lambda i: i['id'] == symbol, self.exchange.fetch_markets()))
        if info_list:
            return info_list[0]
        else:
    ...
    def setleverage(self, symbol, leverage):
        return float(self.exchange.fapiPrivate_post_leverage({'symbol': symbol, 'leverage': leverage})['leverage'])
    ...
    @retry
    def fetch_order_trades(self, symbol, order_id):
        trades = self.exchange.fetch_my_trades(symbol=symbol)
        return list(filter(lambda i: i['order'] == order_id, trades))
