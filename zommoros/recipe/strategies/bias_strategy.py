...
from zommoros.recipe import indicators as myind
...
class BiasStrategy(bt.Strategy):
    ...
    def __init__(self):
        self.macd = btind.MACD(self.datas[1])
        self.bias = myind.BIAS(self.datas[2])
        self.boll = btind.BollingerBands(self.datas[2])
        self.order = None
    ...
    def next(self):
        ...
        if not self.position:
            open_long_signal = self.bias.long[-1] < self.bias.short[-1] and \
                             self.bias.long[0] > self.bias.short[0] and \
                            (self.boll.mid[-2] < self.boll.mid[-1] < self.boll.mid[0]) and \
                             self.macd.macd > self.macd.signal
                             
            open_short_signal = self.bias.long[-1] > self.bias.short[-1] and \
                              self.bias.long[0] < self.bias.short[0] and \
                             (self.boll.mid[-2] > self.boll.mid[-1] > self.boll.mid[0]) and \
                              self.macd.macd < self.macd.signal
            ...
            close_long_signal = self.extreme_price is not None and \
                              self.data.close[0] <= self.extreme_price * (1 - self.p.retracement_ratio)
